...
public class CHybrid implements Constructive<KBInstance, KBSolution> {
    ...
    public KBSolution constructSolution(KBInstance instance) {
        ...
        while (!sol.isFeasible()) {
            ...
            sol.add(c.f);
            if (rnd.nextBoolean()) {
                updateCLBalance(sol, cl);
            } else {
                updateCLDistance(sol, cl);
            }
        }
        return sol;
    }
    ...
    the load.
     */
    private void updateCLBalance(KBSolution sol, List<Candidate> cl) {
        KBInstance instance = sol.getInstance();
        int n = instance.getN();
     ...
     */
    private void updateCLDistance(KBSolution sol, List<Candidate> cl) {
        KBInstance instance = sol.getInstance();
        int n = instance.getN();
        for (Candidate c : cl) {
            c.cost = 0;
