...
public class KBalanceProblem implements Problem {
    ...
    public KBalanceProblem(String path) {
        super();
        load(path);
    }
    ...
    private int[] evaluate(int[] facilities) {
        ...
        for (Map.Entry<Integer, Integer> entry : assigned.entrySet()) {
            minAss = Math.min(entry.getValue(), minAss);
            maxAss = Math.max(entry.getValue(), maxAss);
        }
        return new int[]{of, maxAss, maxAss-minAss};
    }
    ...
    public Solution newSolution() {
        Solution solution = new Solution(k, 3, 1);
        for (int i = 0; i < k; i++) {
